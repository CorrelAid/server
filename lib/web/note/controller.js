...
exports.showAllNotesOverview = function (req, res) {
  let authStatus = req.isAuthenticated()
  let deleteToken = ''

  // get all notes
  noteUtil.findAllNotes(req, res, function (notes) {
    // sort by tags
    let sorted = {
      untagged: []
    }
    notes.forEach(note => {
      if (!note['tags'].length) {
        sorted['untagged'].push(note)
      } else {
        note['tags'].forEach(tag => {
          if (!sorted[tag]) {
            sorted[tag] = []
          }
          sorted[tag].push(note)
        })
      }
    })
    ...
    let data = {
      ...
      notesSortedByTag: sorted,
      activeHeader: 'allnotes'
    }
